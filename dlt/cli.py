...
def _fmt(note: dict) -> str:
    ...
    return f"前区 {reds}   后区 {blues}"


def main(argv=None):
    p = argparse.ArgumentParser(description="大乐透 智能选号（10注 + 冷热/均衡/扰动）")
    ...
    if not records:
        print("⚠️  未获取到任何大乐透历史数据，无法选号。请检查网络或 data/dlt_history.json。")
        return
    ...
    print("=" * 58)
    print("        大乐透 智能选号（%d 注）  %s" % (args.notes, today))
    print("=" * 58)
    ...
    print("\n【选号策略说明】")
    print("  · 冷热号分析：前区热号 %s；前区冷号(高遗漏) %s。" % (hot_r, cold_r))
    print("    后区热号 %s；后区冷号(高遗漏) %s。以热号为主、冷号补足。" % (hot_b, cold_b))
    print("  · 奇偶均衡：前区 奇%d/偶%d，后区 奇%d/偶%d（目标≈50:50）。"
          % (bal["r_odd"], bal["r_even"], bal["b_odd"], bal["b_even"]))
    print("  · 大小均衡：前区 大(≥%d)%d/小%d，后区 大(≥%d)%d/小%d（目标≈50:50）。"
...
def _write_report(today, last, meta, notes, hc, bal, n_notes):
    try:
        ...
        for i, n in enumerate(notes, 1):
            lines.append("%d. %s\n" % (i, _fmt(n)))
        lines.append("\n## 选号策略\n")
        lines.append("- 前区热号：%s；前区冷号：%s\n"
                     % (" ".join("%02d" % d for d in hc["hot_red"]),
                        " ".join("%02d" % d for d in hc["cold_red"])))
        lines.append("- 后区热号：%s；后区冷号：%s\n"
                     % (" ".join("%02d" % d for d in hc["hot_blue"]),
                        " ".join("%02d" % d for d in hc["cold_blue"])))
        lines.append("- 奇偶均衡：前区 奇%d/偶%d，后区 奇%d/偶%d\n"
                     % (bal["r_odd"], bal["r_even"], bal["b_odd"], bal["b_even"]))
        lines.append("- 大小均衡：前区 大%d/小%d，后区 大%d/小%d\n"
